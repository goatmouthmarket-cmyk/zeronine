...
function profileTarget(ctx: RecoveryContext): number {
  // Conservative keeps the market selection narrow, not the recovery target:
  // once a safe extreme is selected, one win must repay the outstanding debt.
  if (ctx.strategy === 'conservative') return ctx.debt;
